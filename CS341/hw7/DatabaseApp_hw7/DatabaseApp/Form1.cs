...
namespace DatabaseApp
{
    ...
    //
		...
		{
			string connectionInfo = "Data Source=" + txtFileName.Text;
    ...
    //
      ...
      else
      {
		...
		private void cmdInsertMovie_Click(object sender, EventArgs e)
		{
			...
			//
			// first we need to find max existing id, and add one:
			//
			...
			cmd.CommandText = string.Format("INSERT INTO Movies(MovieID, MovieName) Values({0}, '{1}');",
				maxid,
				name);
			...
			{
				listBox1.Items.Add("New movie ID: " + maxid.ToString());
			}
			else
			{
